...
func NewHumanizeDb() HumanizeDB {
	...
	// Drunkard prompts + rule-sets
	_, err = db.CreatePrompt(prompts.DrunkardAngryPrompt, nil)
	if err != nil {
		logrus.Error("could not save the drunkard angry prompt")
	}
	_, err = db.CreatePrompt(prompts.DrunkardNormalPrompt, nil)
	if err != nil {
		logrus.Error("could not save the drunkard normal prompt")
	}
	_, err = db.CreatePromptSegment(prompts.DrunkardEmotionalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the drunkard emotional primer")
	}
	_, err = db.CreatePromptSegment(prompts.DrunkardAngryEmotionalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the drunkard angry emotional primer ")
	}

	// Elijah prompts + rule - sets
	_, err = db.CreatePrompt(prompts.ElijahNormalPrompt, nil)
	if err != nil {
		logrus.Error("could not create the elijah normal prompt")
	}
	_, err = db.CreatePrompt(prompts.ElijahAngryPrompt, nil)
	if err != nil {
		logrus.Error("could not save the elijah angry primer")
	}
	_, err = db.CreatePromptSegment(prompts.ElijahEmotionalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the elijah emotional primer")
	}
	_, err = db.CreatePromptSegment(prompts.ElijahNormalMotivationalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the elijah motivational primer")
	}
	_, err = db.CreatePromptSegment(prompts.ElijahMotivationalPrimerAngry, nil)
	if err != nil {
		logrus.Error("could not save the elijah angry motivational primer")
	}
	_, err = db.CreatePromptSegment(prompts.ElijahMotivationalPrimerVeryAngry, nil)
	if err != nil {
		logrus.Error("could not save the elijah very angry motivational primer")
	}
	_, err = db.CreatePromptSegment(prompts.ElijahHappyMotivationalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the elijah happy motivational primer")
	}
	_, err = db.CreatePromptSegment(prompts.ElijahVeryHappyMotivationalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the elijah very happy motivational primer")
	}

	// Erik prompts + rule - sets
	_, err = db.CreatePrompt(prompts.ErikNormalPrompt, nil)
	if err != nil {
		logrus.Error("could not create the erik normal prompt")
	}
	_, err = db.CreatePromptSegment(prompts.ErikEmotionalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the erik emotional primer")
	}
	_, err = db.CreatePromptSegment(prompts.ErikNormalMotivationalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the erik normal motivational primer")
	}
	_, err = db.CreatePromptSegment(prompts.ErikMotivationalPrimerAngry, nil)
	if err != nil {
		logrus.Error("could not save the erik angry motivational primer")
	}

	// Ullr prompts + rule - sets
	_, err = db.CreatePrompt(prompts.UllrPrompt, nil)
	if err != nil {
		logrus.Error("could not create the ullr normal prompt")
	}
	_, err = db.CreatePromptSegment(prompts.UllrEmotionalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the ullr emotional primer")
	}
	_, err = db.CreatePromptSegment(prompts.UllrNormalMotivationalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the ullr normal motivational primer")
	}
	_, err = db.CreatePromptSegment(prompts.UllrMotivationalPrimerAngry, nil)
	if err != nil {
		logrus.Error("could not save the ullr angry motivational primer")
	}
	_, err = db.CreatePrompt(prompts.ElderNormalPrompt, nil)
	if err != nil {
		logrus.Error("could not save the elder normal prompt")
	}
	_, err = db.CreatePromptSegment(prompts.ElderNormalMotivationalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the elder normal motivational primer")
	}
	_, err = db.CreatePromptSegment(prompts.ElderNormalEmotionalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the elder normal emotional primer")
	}

	_, err = db.CreatePrompt(prompts.PaganNormalPrompt, nil)
	if err != nil {
		logrus.Error("could not save the pagan normal prompt")
	}
	_, err = db.CreatePromptSegment(prompts.PaganNormalMotivationalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the pagan normal motivational primer")
	}
	_, err = db.CreatePromptSegment(prompts.PaganNormalEmotionalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the pagan normal emotional primer")
	}

	_, err = db.CreatePrompt(prompts.LoyalistNormalPrompt, nil)
	if err != nil {
		logrus.Error("could not save the loyalist normal prompt")
	}
	_, err = db.CreatePromptSegment(prompts.LoyalistNormalMotivationalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the loyalist normal motivational primer")
	}
	_, err = db.CreatePromptSegment(prompts.LoyalistNormalEmotionalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the loyalist normal emotional primer")
	}

	_, err = db.CreatePrompt(prompts.SofiaNormalPrompt, nil)
	if err != nil {
		logrus.Error("could not save the sofia normal prompt")
	}
	_, err = db.CreatePromptSegment(prompts.SofiaNormalEmotionalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the sofia normal motivational primer")
	}
	_, err = db.CreatePromptSegment(prompts.SlightlyAngryMotivationalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the sofia normal motivational primer")
	}
	_, err = db.CreatePromptSegment(prompts.SophiaVeryAngryEmotionalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the sofia normal motivational primer")
	}
	_, err = db.CreatePromptSegment(prompts.SophiaAngryMotivationalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the sofia normal emotional primer")
	}

	_, err = db.CreatePrompt(prompts.FriendlyWarriorNormalPrompt, nil)
	if err != nil {
		logrus.Error("could not save the friendly warrior normal prompt")
	}
	_, err = db.CreatePromptSegment(prompts.FriendlyWarriorNormalMotivationalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the friendly warrior normal motivational primer")
	}
	_, err = db.CreatePromptSegment(prompts.FriendlyWarriorNormalEmotionalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the friendly warrior normal emotional primer")
	}

	_, err = db.CreatePrompt(prompts.UnfriendlyWarriorNormalPrompt, nil)
	if err != nil {
		logrus.Error("could not save the unfriendly warrior normal prompt")
	}
	_, err = db.CreatePromptSegment(prompts.UnfriendlyWarriorNormalMotivationalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the unfriendly warrior normal motivational primer")
	}
	_, err = db.CreatePromptSegment(prompts.UnfriendlyWarriorNormalEmotionalPrimer, nil)
	if err != nil {
		logrus.Error("could not save the unfriendly warrior normal emotional primer")
	}
