...
type GameLoopManagerRegistry interface {
	AddGameLoop(sessionId string, manager game_loop.GameLoopManager) error
	GetGameLoop(sessionId string) (game_loop.GameLoopManager, error)
	StopGameLoop(sessionId string) error
	PurgeGameLoops() error
}

type GameLoopManagerRegistryImpl struct {
	gameLoopManagers map[string]game_loop.GameLoopManager
}
...
func NewGameLoopManagerRegistry() GameLoopManagerRegistry {
	return &GameLoopManagerRegistryImpl{
		gameLoopManagers: make(map[string]game_loop.GameLoopManager, 0),
	}
}

func (g GameLoopManagerRegistryImpl) AddGameLoop(sessionId string, manager game_loop.GameLoopManager) error {
	g.gameLoopManagers[sessionId] = manager
	return nil
}

func (g GameLoopManagerRegistryImpl) GetGameLoop(sessionId string) (game_loop.GameLoopManager, error) {
