...
func (h *HumanizeDbImpl) CreatePromptSegment(
	...
	createPromptSegment := func(tx *gorm.DB) error {
		...
		p := &PromptSegment{
			...
			EmotionalSquareDifference: rule_system_utilties.CalculateEmotionalStateSquaredDifference(promptSegment.IdealEmotionalState),
		}
		err := tx.Create(p)
		if err != nil {
			if err.Error != nil {
...
func (h *HumanizeDbImpl) GetPromptSegment(
	...
	getPromptSegment := func(tx *gorm.DB) error {
		...
		segment.EmotionalStateSquaredDifference = result.EmotionalSquareDifference
...
func (h *HumanizeDbImpl) ListPromptSegments(
